...
- ✅ **6.1** 预设颜色映射静态 Dictionary → FrozenDictionary（Commit: `dfb1554b`）
- ✅ **6.3** TimerStatistic.GenerateRemainingTimeText Dead Code（Commit: 本次）
...
### 6.3 ✅ ~~TimerStatistic.GenerateRemainingTimeText 中的 Dead Code~~（已修复）

- **文件**：`src/AtomUI.Desktop.Controls/Statistic/TimerStatistic.cs`
- **问题描述**：

`else if (Format == @"hh\:mm\:ss")` 和 `else if (Format == @"hh\:mm\:ss\.fff")` 位于 `if (Format != null)` 之后，此时 `Format` 必然为 `null`，这两个分支永远不会执行。

- **修复方案**：移除死代码分支，改为基于 `RemainingTime.TotalHours` 的智能默认格式，同时消除多余的 `= null` 初始赋值：
